...
| `:sig <symbol|query|expr>` | 現在セッションで見える関数の signature を表示する。名前だけなら定義 signature を返し、typed query を与えたときは specialized 表示を返す。trait / operator は「型なしなら定義、型ありなら実装」とし、`gt(Int, Int)`, `ret |>= up`, `:sig |>`, `:sig id(1)` のような query を受け付ける。operator alias は trait surface へ正規化し、`=` / `=?` は binding form surface (`pattern = expr`, `pattern =? expr`) を返す。helper 名は current scope に従って解決する。auto-import されない helper は qualified path か明示 import 後のみ bare 名で有効とする。`Tuple` は doc 専用 surface なので `:sig Tuple` は無効とする。`pair._1` のような field access sugar は `Lens::view` へ展開した expression query として扱い、他の `Lens::view` / `Lens::set` / `Lens::over` / `Lens::over_result` / `Lens::compose` は通常の関数 query と同じ defined / specialized 表示を返す。 |
