# Surtr Process Runtime 仕様変更一覧 v0
...
この文書は、変更依頼書ではなく、Process Runtime 改修に伴う **仕様変更一覧** である。
...
標準 I/O 差し替えのゴールは、Rust からのテストと Pure Surtr test DSL の双方で同じ意味にすることである。
...
これは目標であり、現時点の仕様変更一覧では内部実装方式を固定しない。
...
この節は、上記仕様変更一覧に対して、現時点で実装済みの項目を追記するためのメモである。

### 7.1 未 push コミットで完了済みの項目

以下はローカルブランチ上で commit 済みだが、まだ push していない変更である。

| commit | 完了項目 |
|---|---|
| `a6670d12` | Process Runtime v2 の compiler contract を導入。`RuntimeProcessSpec` / `RuntimeBootPlan` を compiler 境界で扱う土台を追加。singleton 利用検査、Boot 定義の正規化、process metadata の v2 契約整理を開始。 |
| `8d1981ee` | Process Runtime v2 の schema / validation を拡張。runtime metadata の構造化、BootPlan 検査、handler override 検査、runtime 側が受け取る spec の整合チェックを追加。 |
| `a93285a1` | `DynamicSupervisor` の worker spawn surface を追加。worker init route を supervisor 経由で起動する compiler/runtime の基本経路を実装。 |

### 7.2 現在の作業ツリーで完了済みの項目

以下は commit 前の作業ツリーにあるが、実装と検証まで完了している項目である。

| 項目 | 完了内容 |
|---|---|
| supervisor 定義 surface | `defsupervisor` を policy-only `meta` へ整理し、`instance` / `init_policy` を禁止。`DynamicSupervisor` も同じ `defsupervisor` 形式へ統一。 |
| supervisor policy | `strategy`, `max_restarts`, `max_seconds`, `child_restart_default`, `allow_adopt`, `shutdown_timeout` を parser / AST / runtime spec で保持。Bool 表記は `True` / `False` に統一。 |
| compiler-managed supervisor API | user helper の手書きを禁止し、`spawn(worker_init)`, `adopt(pid)`, `status()` を compiler-managed surface として生成。 |
| `supervisor_init` | singleton boot に加えて supervisor override を受理するよう拡張。`parent` override は明示的に拒否。 |
| typecheck | `Sup::spawn`, `Sup::adopt`, `Sup::status` を専用に型付け。worker init route 制約、`PID<Worker>` 制約、`allow_adopt: False` 拒否、boot plan 収録検査を追加。 |
| codegen / runtime boot plan | `EffectiveSupervisorSpec` 相当の runtime 入力と fixed parent ルールを encode。singleton process は `RuntimeSupervisor` 配下、custom / dynamic supervisor は `RootSupervisor` 配下へ固定。 |
| Eldr runtime | supervisor identity を runtime 側で扱い、spawn / adopt / status の builtin 経路を追加。child table と effective policy を保持し、adopt 時に lifecycle sink を付け替えるよう更新。 |
| stdlib / spec fixture | `lib/process.srt` に `SupervisorStatus` を追加。supervisor user surface fixture と runtime spec 文書を v0 仕様へ更新。 |

### 7.3 検証結果

今回の local 実装反映後、次の検証は完了している。

- `cargo nextest run --workspace`
- 結果: `1102 passed, 0 skipped`
