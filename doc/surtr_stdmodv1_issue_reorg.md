...
- **コミット粒度を小さく保つ**
  - 1コミットで扱う関心事は原則1つに限定する
  - 「基盤修正」と「既存テスト互換のための調整」は分離してコミットする
- **現時点でルートが1本の経路は維持してよい**
  - 実装途中で無理に経路分岐を増やさない
  - 新設API経由でも既存ルート修正でも、差分が小さくなる方を優先する
- **既存APIの破壊的変更は後段に集約する**
  - `Issue A〜F` の完了までは互換レイヤー/既存APIを残してよい
  - `Issue A〜F` 完了後に、破壊的変更フェーズを設けて全面切替を行う
- **段階移行を明示する**
  - 各Issueで「新設」「互換維持」「最終切替」のどの段階かを明記する
  - 互換維持のために残すコードは、後で削除する前提をコメント/Issueで追跡する
...
- 後続の追補タスクは [doc/surtr_stdmodv1_followup_tasks.md](./surtr_stdmodv1_followup_tasks.md) に切り出して管理する。本書は再編方針と実装済み経緯の参照元として維持する。
