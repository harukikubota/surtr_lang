use serde::{de, de::VariantAccess, Deserialize, Deserializer, Serialize, Serializer};
...
impl Serialize for RuntimeProcessKind {
        ...
        S: Serializer,
    {
        match self {
            RuntimeProcessKind::Agent => {
                serializer.serialize_unit_variant("RuntimeProcessKind", 0, "Agent")
            }
            RuntimeProcessKind::GenServer => {
                serializer.serialize_unit_variant("RuntimeProcessKind", 2, "GenServer")
            }
            RuntimeProcessKind::Supervisor => {
                serializer.serialize_unit_variant("RuntimeProcessKind", 3, "Supervisor")
            }
            RuntimeProcessKind::RuntimeSupervisor => {
                serializer.serialize_unit_variant("RuntimeProcessKind", 4, "RuntimeSupervisor")
            }
            RuntimeProcessKind::DynamicSupervisor => {
                serializer.serialize_unit_variant("RuntimeProcessKind", 5, "DynamicSupervisor")
            }
            RuntimeProcessKind::Task => {
                serializer.serialize_unit_variant("RuntimeProcessKind", 6, "Task")
            }
        }
    }
}

impl<'de> Deserialize<'de> for RuntimeProcessKind {
        ...
        D: Deserializer<'de>,
    {
                ...
                D: Deserializer<'de>,
            {
                ...
                impl de::Visitor<'_> for FieldVisitor {
                    ...
                    fn expecting(
                        ...
                        formatter.write_str("a runtime process kind")
                    }
                    ...
                    {
                        match value {
                            "Agent" | "ReadOnlyAgent" => Ok(Field::Agent),
                            "StateAgent" => Ok(Field::LegacyStateAgent),
                            "GenServer" => Ok(Field::GenServer),
                            "Supervisor" => Ok(Field::Supervisor),
                            "RuntimeSupervisor" => Ok(Field::RuntimeSupervisor),
                            "DynamicSupervisor" => Ok(Field::DynamicSupervisor),
                            "Task" => Ok(Field::Task),
                            other => Err(de::Error::unknown_variant(
                                other,
                                RUNTIME_PROCESS_KIND_VARIANTS,
                            )),
                        }
                    }
                        ...
                        E: de::Error,
                    {
                        match value {
                            0 | 1 => Ok(Field::Agent),
                            2 => Ok(Field::GenServer),
                            3 => Ok(Field::Supervisor),
                            4 => Ok(Field::RuntimeSupervisor),
                            5 => Ok(Field::DynamicSupervisor),
                            6 => Ok(Field::Task),
                            other => Err(de::Error::invalid_value(
                                de::Unexpected::Unsigned(other),
                                &"a valid runtime process kind variant index",
                            )),
                        }
                    }
                }
        ...
        impl<'de> de::Visitor<'de> for KindVisitor {
            type Value = RuntimeProcessKind;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("enum RuntimeProcessKind")
            }
                ...
                A: de::EnumAccess<'de>,
            {
                ...
                Ok(match field {
                    Field::Agent => RuntimeProcessKind::Agent,
                    Field::LegacyStateAgent => RuntimeProcessKind::Agent,
                    Field::GenServer => RuntimeProcessKind::GenServer,
                    Field::Supervisor => RuntimeProcessKind::Supervisor,
                    Field::RuntimeSupervisor => RuntimeProcessKind::RuntimeSupervisor,
                    Field::DynamicSupervisor => RuntimeProcessKind::DynamicSupervisor,
                    Field::Task => RuntimeProcessKind::Task,
                })
            }
        }
...
const RUNTIME_PROCESS_KIND_VARIANTS: &[&str] = &[
    "Agent",
    "StateAgent",
    "GenServer",
    "Supervisor",
...
impl Serialize for RuntimeProcessInstance {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        match self {
            RuntimeProcessInstance::Singleton => {
                serializer.serialize_unit_variant("RuntimeProcessInstance", 0, "Singleton")
            }
            RuntimeProcessInstance::Worker => {
                serializer.serialize_unit_variant("RuntimeProcessInstance", 2, "Worker")
            }
        }
    }
}

impl<'de> Deserialize<'de> for RuntimeProcessInstance {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
                ...
                D: Deserializer<'de>,
            {
                ...
                impl de::Visitor<'_> for FieldVisitor {
                    ...
                    fn expecting(
                        ...
                        formatter.write_str("a runtime process instance")
                    }
                    ...
                    {
                        match value {
                            "Singleton" => Ok(Field::Singleton),
                            "Multi" => Ok(Field::LegacyMulti),
                            "Worker" => Ok(Field::Worker),
                            other => Err(de::Error::unknown_variant(
                                other,
                                RUNTIME_PROCESS_INSTANCE_VARIANTS,
                            )),
                        }
                    }
                    ...
                    {
                        match value {
                            0 => Ok(Field::Singleton),
                            1 => Ok(Field::LegacyMulti),
                            2 => Ok(Field::Worker),
                            other => Err(de::Error::invalid_value(
                                de::Unexpected::Unsigned(other),
                                &"a valid runtime process instance variant index",
                            )),
                        }
                    }
                }
        ...
        impl<'de> de::Visitor<'de> for InstanceVisitor {
            type Value = RuntimeProcessInstance;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("enum RuntimeProcessInstance")
            }
                ...
                A: de::EnumAccess<'de>,
            {
                ...
                variant.unit_variant()?;
                Ok(match field {
                    Field::Singleton => RuntimeProcessInstance::Singleton,
                    Field::LegacyMulti => RuntimeProcessInstance::Worker,
                    Field::Worker => RuntimeProcessInstance::Worker,
                })
            }
        }
...
const RUNTIME_PROCESS_INSTANCE_VARIANTS: &[&str] = &["Singleton", "Multi", "Worker"];
...
mod tests {
    ...
    #[test]
    fn runtime_process_schema_decodes_legacy_agent_names() {
        let readonly: RuntimeProcessSpec = serde_json::from_str(
            r#"{
              "process_name": "Config",
              "module_path": "Agents::Config",
              "kind": "ReadOnlyAgent",
              "instance": "Multi",
              "boot": false,
              "registry": false,
        ...
        .expect("legacy readonly agent spec should decode");
        assert_eq!(readonly.kind, RuntimeProcessKind::Agent);
        assert_eq!(readonly.instance, RuntimeProcessInstance::Worker);

        let stateful: RuntimeProcessSpec = serde_json::from_str(
            r#"{
              "process_name": "Counter",
              "module_path": "Agents::Counter",
              "kind": "StateAgent",
              "instance": "Singleton",
              "boot": true,
              "registry": true,
        ...
        .expect("legacy state agent spec should decode");
        assert_eq!(stateful.kind, RuntimeProcessKind::Agent);
        assert_eq!(stateful.instance, RuntimeProcessInstance::Singleton);
    }

    #[test]
    fn runtime_process_schema_decodes_legacy_bincode_variants() {
        ...
        #[derive(serde::Serialize)]
        struct LegacyRuntimeProcessSpec {
            process_name: String,
            module_path: String,
            kind: LegacyKind,
        ...
        let readonly = LegacyRuntimeProcessSpec {
            process_name: "Config".into(),
            module_path: "Agents::Config".into(),
            kind: LegacyKind::ReadOnlyAgent,
            instance: LegacyInstance::Multi,
            boot: false,
            registry: false,
            lazy: true,
            init_fun_idx: 0,
        ...
        let decoded: RuntimeProcessSpec = bincode::deserialize(
            &bincode::serialize(&readonly).expect("legacy readonly spec should encode"),
        )
        .expect("legacy readonly bincode should decode");
        assert_eq!(decoded.kind, RuntimeProcessKind::Agent);
        assert_eq!(decoded.instance, RuntimeProcessInstance::Worker);

        let stateful = LegacyRuntimeProcessSpec {
            process_name: "Counter".into(),
            module_path: "Agents::Counter".into(),
            kind: LegacyKind::StateAgent,
            instance: LegacyInstance::Singleton,
            boot: true,
            registry: true,
            lazy: false,
            init_fun_idx: 0,
        ...
        let decoded: RuntimeProcessSpec = bincode::deserialize(
            &bincode::serialize(&stateful).expect("legacy stateful spec should encode"),
        )
        .expect("legacy stateful bincode should decode");
        assert_eq!(decoded.kind, RuntimeProcessKind::Agent);
        assert_eq!(decoded.instance, RuntimeProcessInstance::Singleton);
    }
