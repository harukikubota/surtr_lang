...
- 仕様補足として process runtime の補助仕様を整備し、`docs/dev/EldrVM_spec.md` に PID と隠し builtin の位置づけを追記する。
...
次フェーズでは [Process runtime spec](./dev/ProcessRuntime_spec.md) を正として、Spire lowering だけに閉じた実装から VM の process object model へ段階的に寄せるのがよいです。
